...
2028 → 2029 → los 12 WIGs. Cada slide de WIG muestra su sección **«Temas
puntuales»** (`temasHTML`): las metas binarias con fecha de ese WIG, ordenadas
por estado y fecha. El slide de **compañía** ($1M NAT) incluye una rejilla
**clicable** de los 12 WIGs (`wigStatusHTML`) con el semáforo «¿vamos en meta?»
(de `verdictOf`); tocar un WIG abre su slide (`data-go` = índice del slide).
`Compromisos` ya **no se genera** pero el parser
...
| Temas puntuales (bloque bajo los datos) | **Metas binarias con fecha objetivo**. Rótulo `Temas puntuales…` (legado `Hitos…`) en col A en `último_dato + 2`; encabezados en la fila siguiente; `HITO_ROWS` (6) filas de entrada. Cols: **A–D Tema** (combinadas) · **E Lead** (1–8, opcional) · **F Responsable** · **G Fecha objetivo** · **H Estado** (`Pendiente`/`En curso`/`Logrado`). El parser lo localiza por el rótulo (regex `^(temas? puntual|hitos)`, no por fila fija, porque `último_dato` varía por WIG). |
...
- Temas puntuales por WIG: encabezados en `48+3+1`, una fila por WIG. Por WIG:
  # · WIG · Logrado · En curso · Pendiente (`COUNTIF` sobre el rango de Estado del
  bloque de Temas puntuales de la pestaña) · Próxima fecha (`MIN` de las fechas
  objetivo). Las fórmulas referencian el rango de cada WIG, que `build_wig.py`
  conoce en tiempo de construcción (`hitos_ranges`).
...
Service Provider") **no** van en Tareas: viven en el bloque **Temas puntuales**
de cada pestaña de WIG (ver arriba).
...
- Captura del dato (3 campos): **equipo** (lag = E4; por lead = fila 5),
  **responsable** (lag = B6; por lead = fila 6), **fuente/método** (lag = B7; por
  lead = fila 7). Temas puntuales: estados `Pendiente`/`En curso`/`Logrado` (texto exacto,
