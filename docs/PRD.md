...
The company WIG for this cycle is:

> **Generate $1,000,000 of net profit after taxes (NAT) in 2027.**

That single number is too lagging to act on weekly, so it is supported by **9 WIGs**,
each owned by a function, each with its own lag measure and up to 8 lead measures (see
§4). Discipline 3 ("keep a compelling scoreboard") is where software comes in: without a
...
1. **It is trivial for the owners to update.** Nine functional owners enter their numbers
...
- $1M NAT in 2027, decomposed into the 9 WIGs in §4. The product does not *produce* this
  number — it makes the team's progress toward it visible and actionable every week.

### Non-goals

- **Not a system of record / BI platform.** Excel is the source of data on purpose, until
  the cadence matures (see §9 and `CLAUDE.md` "Decisiones tomadas"). No Airtable/Sheets/
  Power BI today.
...
- **Not public.** The scoreboard contains internal targets and performance. It is
  restricted to the office (LAN via PGX; IP-allowlisted on Azure). It is not a marketing
  or external-stakeholder artifact.
...
## 4. The 9 supporting WIGs (product content)
...
- **Cumulative vs. level WIGs.** Cumulative WIGs (1, 2, 3, 5, 7) accumulate toward a total;
  level WIGs (4, 6, 8, 9) must hold at/under/over a threshold each period.
- **"Less is better" WIGs.** WIG 6 (expired contracts → 0) inverts the scoring; the parser
  detects it via `/expired/i`. Its lead measures are still phrased *positively* (more = better)
  so the percentage stays comparable across all WIGs.
...
2. On their WIG tab, types the week's **Real** value (column D) into the blue input cells;
   optionally updates lead-measure actuals and adds rows to the **Compromisos** sheet.
...
1. The office TV runs `marcador.html` in kiosk Chrome, cycling slides: company NAT dashboard,
   then one slide per WIG with its lag gauge, lead visualizations, and color status.
...
3. Tapping/clicking a lead opens its **detail**: the indicator's history + that lead's
   commitments from the Compromisos sheet. Light/dark theme via the ◐ button or `T`
   (persisted).
...
- **FR4 — Company dashboard.** First sheet shows annual NAT target (C4) and months 7–18
  (meta, real, accumulated, %, status), plus a roll-up of all 9 WIGs' latest status.
...
- **FR6 — Commitments drill-down.** Optional **Compromisos** sheet (`Semana | WIG | Lead |
  Compromiso | Responsable | Estado`, `Estado ∈ {Pendiente, Hecho}`) feeds each lead's detail
  view. Absent sheet = no commitments shown (must not break the parser — contract-tested).
...
- **NFR3 — Security / confidentiality.** Internal targets and performance. LAN-only via PGX;
  on Azure, IP-allowlisted to the office (`networking.allowedIpRanges`, Standard tier ~$9/mo).
  The deploy workflow refuses to publish while the IP placeholder remains.
...
- **M1 — Data freshness.** % of the 9 WIGs with current-week/month data entered by each Monday
  meeting. *Target: 9/9 (100%) by meeting time, sustained.*
...
| Public exposure of internal targets | Confidentiality breach | LAN-only / IP allowlist; workflow refuses placeholder IP (NFR3) |
...
- [`excel/build_wig.py`](../excel/build_wig.py) — single source of truth for workbook structure and the 9 WIGs.
